# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/02.00.develop.utils.ipynb (unless otherwise specified).

__all__ = ['listify', 'PASSING_STATUS', 'FAILING_STATUS', 'FormatValidation', 'format_validation', 'find_package',
           'find_code', 'ValidateCode', 'validate_code', 'ValidateRecord', 'validate_records', 'store', 'start',
           'increment_version', 'string_version', 'revise']
...
# Cell
def find_package(package, parent=None):
    """Find a package through imports."""
    if package is None: return parent
    if isinstance(package, ModuleType): return package
    target, *remainder = str(package).split(".")

    if not parent is None:
        if hasattr(parent, target):
            parent = getattr(parent, target)
            if len(remainder) == 0: return parent
            package = ".".join(remainder)
            return find_package(package, parent=parent)
        return None

    try:
        parent = __import__(target)
        if len(remainder) == 0: return parent
        package = ".".join(remainder)
        return find_package(package, parent=parent)
    except:
...
def find_code(name, package=None):
    """Use text descriptions to find executable code."""
    g = globals()
    ...
    if hasattr(package, name): return getattr(package, name)
...
class ValidateCode:
    ...
    def fn(self):
        ...
        name = self.kw.get('fn', self.name)
        self._fn = find_code(name, package=self.package)
        return self._fn
